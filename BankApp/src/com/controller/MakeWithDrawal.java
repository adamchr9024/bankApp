...
/**
 * Servlet implementation class MakeWithDrawal
 */
@WebServlet("/makewithdrawal")
public class MakeWithDrawal extends HttpServlet {
	...
	//	System.out.println("account number = "+accountholder.getAcct().getAccountNo());
		...
		try {
			long updated = checkingservice.withdrawal(accountholder.getAcct().getAccountNo(), amount);
			rd=request.getRequestDispatcher("success");
			rd.include(request, response);
			if (updated > 0) {
				pw.print("<center><span style='color:green;'>Withdrawal Successful</span></center>");
			}
			else
